...
STATICFILES_STORAGE = os.environ.get('STATICFILES_STORAGE', s('STATICFILES_STORAGE'))
...
MKTG_URL_OVERRIDES.update(__config__.get('MKTG_URL_OVERRIDES', MKTG_URL_OVERRIDES))

# Intentional defaults.
s('SUPPORT_SITE_LINK')
s('ID_VERIFICATION_SUPPORT_LINK')
...
LOGGING = get_logger_config(s('LOG_DIR',set_global=False),
                            ...
                            local_loglevel=s('LOCAL_LOGLEVEL',set_global=False),
...
if FEATURES.get('ENABLE_THIRD_PARTY_AUTH'):
    ...
    THIRD_PARTY_AUTH_OLD_CONFIG = s('THIRD_PARTY_AUTH')

    if s('THIRD_PARTY_AUTH_SAML_FETCH_PERIOD_HOURS', 24) is not None:
        CELERYBEAT_SCHEDULE['refresh-saml-metadata'] = {
            'task': 'third_party_auth.fetch_saml_metadata',
            'schedule': datetime.timedelta(hours=THIRD_PARTY_AUTH_SAML_FETCH_PERIOD_HOURS),
        }
...
s('PARENTAL_CONSENT_AGE_LIMIT', None)
