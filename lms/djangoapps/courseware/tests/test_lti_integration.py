...
class TestLTI(BaseTestXmodule):
    ...
    def setUp(self):
        ...
        context_id = str(self.item_block.course_id)
        ...
        resource_link_id = str(urllib.parse.quote(f'{hostname}-{self.item_block.location.html_id()}'))
        ...
        self.expected_context = {
            ...
            'element_class': self.item_block.category,
            ...
            'comment': '',
            'weight': 1.0,
            'ask_to_send_username': self.item_block.ask_to_send_username,
            'ask_to_send_email': self.item_block.ask_to_send_email,
            'description': self.item_block.description,
            'button_text': self.item_block.button_text,
            'accept_grades_past_due': self.item_block.accept_grades_past_due,
        }
    ...
    def test_lti_constructor(self):
        generated_content = self.item_block.render(STUDENT_VIEW).content
        expected_content = self.runtime.render_template('lti.html', self.expected_context)
        assert generated_content == expected_content

    def test_lti_preview_handler(self):
        generated_content = self.item_block.preview_handler(None, None).body
