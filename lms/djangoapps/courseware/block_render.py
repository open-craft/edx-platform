...
def get_module_system_for_user(
    ...
    """
    Helper function that returns a module system and student_data bound to a user and a block.
    ...
    are all the other arguments.  Ultimately, this isn't too different than how get_block_for_object_internal
    ...
    Returns:
        (LmsModuleSystem, KvsFieldData):  (module system, student_data) bound to, primarily, the user and block
    """
    ...
    def inner_get_block(block):
        """
        Delegate to get_block_for_object_internal() with all values except `block` set.
    ...
    system = LmsModuleSystem(
        ...
        # TODO: When we merge the block and module systems, we can stop reaching into the mixologist (cpennington)
        mixins=block.runtime.mixologist._mixins,  # pylint: disable=protected-access
    ...
    system.set('days_early_for_beta', block.days_early_for_beta)
