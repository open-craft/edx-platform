...
class TestOverrideScoreInstructorTask(TestInstructorTasks):
    ...
    def test_overriding_non_scorable(self):
        """
        Tests that override problem score raises an error if module block has not `set_score` method.
        """
        ...
        with patch(
                'lms.djangoapps.instructor_task.tasks_helper.module_state.get_block_for_object_internal'
        ) as mock_get_block:
    ...
    def test_overriding_unaccessable(self):
        ...
        self._create_students_with_state(num_students, input_state)
        task_entry = self._create_input_entry(score=0)
        with patch('lms.djangoapps.instructor_task.tasks_helper.module_state.get_block_for_object_internal',
                   return_value=None):
            self._run_task_with_mock_celery(override_problem_score, task_entry.id, task_entry.task_id)
...
class TestRescoreInstructorTask(TestInstructorTasks):
    ...
    def test_rescoring_unrescorable(self):
        ...
        with patch('lms.djangoapps.instructor_task.tasks_helper.module_state.get_block_for_object_internal') as mock_get_block:  # lint-amnesty, pylint: disable=line-too-long
            mock_get_block.return_value = mock_instance
            with pytest.raises(UpdateProblemModuleStateError):
    ...
    def test_rescoring_unaccessable(self):
        ...
        self._create_students_with_state(num_students, input_state)
        task_entry = self._create_input_entry()
        with patch('lms.djangoapps.instructor_task.tasks_helper.module_state.get_block_for_object_internal', return_value=None):  # lint-amnesty, pylint: disable=line-too-long
            self._run_task_with_mock_celery(rescore_problem, task_entry.id, task_entry.task_id)
