...
def _import_xml_node_to_parent(
    ...
    if not children_handled:
        for child_node in child_nodes:
            child_copied_from_block = _get_usage_key_from_node(node, copied_from_block) if copied_from_block else None
            _import_xml_node_to_parent(
                child_node,
                new_xblock,
                store,
                user_id=user_id,
                copied_from_block=str(child_copied_from_block),
                tags=tags,
            )
