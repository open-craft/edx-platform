    ...
    if event == EnrollStatusChange.enroll:
        ...
        context = {
            ...
            'course_url': u"{protocol}://{domain}{path}".format(
                ...
                domain=site_domain,
                path=reverse('about_course', args=[course_id.to_deprecated_string()])
            ),
            'user_admin_url': u"{protocol}://{domain}{path}".format(
                protocol=site_protocol,
                domain=site_domain,
                path=reverse('admin:auth_user_change', args=[user.id]),
            ),
        }
        subject = ''.join(
