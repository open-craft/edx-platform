...
def is_fingerprint_in_bucket(fingerprint, s3_bucket_name):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchBucket":
            ...
            print("S3 key {} does not exist in S3 bucket {}".format(s3_key, bucket_name))
        else:
...
def get_file_from_s3(s3_bucket_name, zipfile_name, path):
    """
    Get the file from s3 and save it to disk.
    """
    print("Retrieving {} from bucket {}.".format(zipfile_name, s3_bucket_name))
    client = boto3.client("s3")

    try:
        bucket = client.head_bucket(Bucket=s3_bucket_name)
    except botocore.exceptions.ClientError as e:
        if e.response["Error"]["Code"] == "NoSuchBucket":
            msg = "Did not find S3 bucket {}".format(
                zipfile_name, s3_bucket_name
            )
            raise BuildFailure(msg)
    ...
    try:
        client.download_file(Bucket=s3_bucket_name, Key=zipfile_name, Filename=zipfile_path)
    except botocore.exceptions.ClientError as e:
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchBucket":
            ...
            print("Unable to upload cache file to S3. "
                  "Continuing without uploading the new cache to S3. Code: %s, Message: %s" % (e.response["Error"]["Code"], e.response["Error"]["Message"]))
        return
